...
export default function NewsletterCapture() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                setStatus("success");
                setMessage("Welcome to Breezi! Check your inbox soon.");
        ...
        <section className="mt-24 mb-10 bg-gradient-to-r from-purple-900/30 via-[#100b20] to-[#050509] border border-purple-500/20 rounded-3xl p-10 md:p-16 text-center max-w-4xl mx-auto relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <form 
                    ...
                    className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto w-full relative"
                >
                    ...
                    <button 
                        type="submit"
                        disabled={status === "loading"}
                        className="bg-white text-black font-bold px-8 py-4 rounded-full hover:bg-gray-200 hover:scale-105 active:scale-95 transition-all whitespace-nowrap disabled:opacity-50"
                    >
                ...
                {/* Status Messages */}
                {status === "success" && (
                    <p className="mt-4 text-purple-400 font-medium animate-pulse">{message}</p>
                )}
                {status === "error" && (
